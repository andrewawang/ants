...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        k=0
        while bee_place.entrance !=hive and ((k<self.min_range or k>self.max_range) or (not bee_place.bees)):
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        "*** REPLACE THIS LINE ***"
        curr_bee=len(self.place.bees)-1
        while curr_bee>=0:
            self.place.bees[curr_bee].reduce_armor(self.damage)
            curr_bee-=1
        # END Problem 6A
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        "*** REPLACE THIS LINE ***"
        QueenAnt.Queen_Quanitity+=1
        if QueenAnt.Queen_Quanitity==1:
    ...
    def action(self, colony):
        ...
        "*** REPLACE THIS LINE ***"
        def double_dmg(ant):
            if isinstance(ant,BodyguardAnt) and ant.ant !=None and ant.ant not in self.doubled_ants:
                ant.ant.damage=ant.ant.damage*2
                self.doubled_ants.append(ant.ant)
                if isinstance(ant,TankAnt) and ant not in self.doubled_ants:
                    ant.damage=ant.damage*2
            elif ant not in self.doubled_ants:
                    ant.damage = ant.damage*2


        if self.one_true_queen==False:
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        "*** REPLACE THIS LINE ***"
        if amount>=self.armor and self.one_true_queen==True:
            bees_win()
